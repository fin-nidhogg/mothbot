...
function messageType() {
    return document.querySelector('[name="messageType"]:checked').value;
}
...
function preview() {
    ...
    const thumbnail = embed.thumbnailUrl ? `<img class="preview-thumbnail" src="${escapeHtml(embed.thumbnailUrl)}" alt="Embed thumbnail">` : "";
    const image = embed.imageUrl ? `<img class="preview-image" src="${escapeHtml(embed.imageUrl)}" alt="Embed image">` : "";
    const footer = embed.footer ? `<div class="preview-footer">${escapeHtml(embed.footer)}</div>` : "";
    byId("preview").innerHTML =
        `<div class="preview-copy"><div class="preview-title">${escapeHtml(embed.title || "Embed title")}</div><p>${escapeHtml(embed.description || "Your embed description will appear here.")}</p>${footer}</div>${thumbnail}${image}`;
}
...
async function loadDirectory() {
    try {
        ...
        guilds = data.guilds;
        byId("guildId").innerHTML =
            '<option value="">Select a server…</option>' +
            guilds.map((guild) => `<option value="${guild.id}">${escapeHtml(guild.name)} (${guild.channels.length} channels)</option>`).join("");
...
function updateChannels(selectedChannel = "") {
    ...
    if (selectedChannel && !channels.some((channel) => channel.id === selectedChannel)) {
        byId("channelId").insertAdjacentHTML(
            "beforeend",
            `<option value="${escapeHtml(selectedChannel)}">Unavailable channel (${escapeHtml(selectedChannel)})</option>`,
        );
    }
...
function render() {
    ...
    byId("items").innerHTML = messages.length
        ? messages
              .map(
                  (message) => `
        ...
        <div class="item-actions"><button type="button" class="ghost" data-edit="${message._id}">Edit</button><button type="button" class="danger" data-delete="${message._id}">Delete</button></div></article>`,
              )
              .join("")
...
function edit(id) {
    ...
    byId("id").value = id;
    byId("name").value = message.name;
    byId("guildId").value = message.guildId;
    updateMentionSearch();
    updateChannels(message.channelId);
    byId("content").value = message.content || "";
    renderMessageEditor(message.content || "", message.guildId);
    byId("scheduledFor").value = toLocalInput(message.scheduledFor);
    byId("embedTitle").value = embed.title || "";
    byId("description").value = embed.description || "";
    byId("imageUrl").value = embed.imageUrl || "";
    byId("thumbnailUrl").value = embed.thumbnailUrl || "";
    byId("footer").value = embed.footer || "";
    setColor("#" + (embed.color ?? 5793266).toString(16).padStart(6, "0"));
    populateRecurrence(message.recurrence);
    setEditorMode(true);
    updateType();
    updateCounters();
    showView("create");
...
byId("colorHex").addEventListener("input", (event) => {
    if (/^#[0-9a-f]{6}$/i.test(event.target.value)) setColor(event.target.value);
});
byId("form").addEventListener("input", (event) => {
    if (!["color", "colorHex"].includes(event.target.id)) preview();
    updateCounters();
});
...
byId("clearButton").addEventListener("click", () => {
    const wasEditing = Boolean(byId("id").value);
    reset();
    if (wasEditing) showView("manage");
});
byId("items").addEventListener("click", async (event) => {
    ...
    if (event.target.dataset.delete && confirm("Delete this scheduled message permanently?")) {
        try {
            await request(`${api}/${event.target.dataset.delete}`, { method: "DELETE" });
            await loadMessages();
        } catch (error) {
...
byId("form").addEventListener("submit", async (event) => {
    ...
    try {
        ...
        reset();
        showView("manage");
        showManageNotice(id ? "Message updated." : "Message created and added to the queue.");
        setNotice("");
    } catch (error) {
        setNotice(error.message, true);
    } finally {
...
function showView(view) {
    ...
    byId("createView").hidden = managing || account;
...
function showManageNotice(message) {
    ...
    showManageNotice.timer = setTimeout(() => {
        notice.hidden = true;
    }, 4000);
}
...
function renderMentionResults() {
    ...
    const matches = mentionCandidates()
        .filter(
            (item) =>
                item.label.toLocaleLowerCase().includes(query) && !selectedMentions.some((selected) => selected.type === item.type && selected.id === item.id),
        )
        .slice(0, 12);
    results.innerHTML = matches.length
        ? matches
              ...
              .join("")
        : "<p>No matching members or roles.</p>";
    results.hidden = false;
}
function renderMentionChips() {
    byId("mentionChips").innerHTML = selectedMentions
        .map(
            (mention, index) =>
...
function extractMentions(content, guildId) {
    ...
    const stripped = content
        .replace(expression, (token, nickname, userId, roleId) => {
...
function syncMessageEditor() {
    byId("content").value = [...byId("messageEditor").childNodes]
...
byId("messageEditor").addEventListener("keydown", (event) => {
    ...
    if (event.key === "ArrowDown" && count) {
        event.preventDefault();
        inlineMentionIndex = (inlineMentionIndex + 1) % count;
        renderInlineMentionResults();
    }
    if (event.key === "ArrowUp" && count) {
        event.preventDefault();
        inlineMentionIndex = (inlineMentionIndex - 1 + count) % count;
        renderInlineMentionResults();
    }
...
function setPasswordNotice(message, error = false) {
    byId("passwordNotice").textContent = message;
    byId("passwordNotice").classList.toggle("error", error);
}
byId("passwordForm").addEventListener("reset", () => setPasswordNotice(""));
byId("passwordForm").addEventListener("submit", async (event) => {
    ...
    const newPasswordConfirmation = byId("confirmPassword").value;
    if (newPassword !== newPasswordConfirmation) {
        setPasswordNotice("New passwords do not match.", true);
    ...
    try {
        const result = await request("/admin-api/auth/change-password", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        setPasswordNotice(error.message, true);
    } finally {
